...
import os
#Debe descargar la librería faker
...
def obtenerEdad(fechaTupla):
    from datetime import date
    hoy = date.today()
    annoNacimiento = fechaTupla[2]
    mesNacimiento = fechaTupla[1]
    diaNacimiento = fechaTupla[0]
    
    edad = hoy.year - annoNacimiento
    if (hoy.month, hoy.day) < (mesNacimiento, diaNacimiento):
        edad -= 1
    return edad
...
def generarDonadores():
    ...
    def generarAux():
        ...
        generados = 0
        colaCorreo = ["gmail.com", "costarricense.cr", "racsa.go.cr", "ccss.sa.cr"]
        for _ in range(cantidad):
            sexo = random.choice([True, False])
            nombre = fake.first_name_male() if sexo == True else fake.first_name_female()
            ...
            correo = f"{nombre.lower()}{random.randint(1,99)}@{random.choice(colaCorreo)}"
            ...
            if estado == 1 and random.random() < 0.1:
                estado = 0
...
def generarReporteRecibirDe(vTipo):
    ...
    puedeRecibir = compatibilidad[tipoSeleccionado]
...
def reportes():
    ...
    #Botoón para reporte por provincia
    botonReporteDonantesProvincia = tk.Button(ventanaReportes, text = "Por Provincia", command= reporteDonantesProvinicia)
    botonReporteDonantesProvincia.pack()
    ...
    botonRangoEdad.pack()
    #Botón para reporte por tipo de sangre
    botonTipoDeSangre = tk.Button(ventanaReportes, text = "Por tipo de sangre", command = reporteTipoSangre)
    botonTipoDeSangre.pack()
    ...
    #Botón para reporte de mujeres donantes O-
    botonMujeresONeg = tk.Button(ventanaReportes, text="Mujeres donantes O-", command = reporteMujeresONeg)
    botonMujeresONeg.pack()
    ...
    #Botón para reporte de quién donar
    botonRecibirDe = tk.Button(ventanaReportes, text="¿De quién puede recibir?", command = reporteRecibirDe)
    botonRecibirDe.pack()
    ...
    botonNoActivos.pack()
    #Botón para reporte de lugares de donación
    botonLugaresDonacion = tk.Button(ventanaReportes, text = "Lugares de donación", command = reporteLugaresDonacion)
    botonLugaresDonacion.pack()
    #Botón para regresar
